...
function SolicitacaoRepasse({ restauranteId }) {
  ...
  async function carregarDados() {
    try {
      ...
      // Definir valor inicial como saldo disponível
  ...
  async function handleSolicitarRepasse(e) {
    ...
    if (!dados.chavePixCadastrada) {
      setError('Você precisa cadastrar uma chave PIX antes de solicitar repasse. Vá em Configurações > Dados Bancários.');
      return;
    }
    ...
    try {
      ...
      setSuccess(`Solicitação de repasse de ${formatCurrency(valorSolicitado)} enviada com sucesso! Prazo: até ${diasSelecionados} dias úteis.`);
  ...
  const getStatusBadge = (status) => {
    const badges = {
      pendente: { color: 'bg-yellow-100 text-yellow-800', label: 'Pendente', icon: ClockIcon },
      processando: { color: 'bg-blue-100 text-blue-800', label: 'Processando', icon: TrendingUpIcon },
      aprovado: { color: 'bg-green-100 text-green-800', label: 'Aprovado', icon: CheckCircleIcon },
      pago: { color: 'bg-green-600 text-white', label: 'Pago', icon: CheckCircleIcon },
      cancelado: { color: 'bg-red-100 text-red-800', label: 'Cancelado', icon: AlertCircleIcon }
    };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="ninja-card p-6 border-2 border-success"
        >
      ...
      {dados.chavePixCadastrada && (
        <motion.div
          ...
          className="ninja-card p-4 bg-blue-50 border border-blue-200"
        >
          <div className="flex items-center justify-between">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex gap-3">
              ...
              <div className="text-sm text-blue-900">
                ...
                {dados.taxaPlataforma > 0 && (
                  <p className="mt-2">
                    Taxa da plataforma: <strong>{(dados.taxaPlataforma * 100).toFixed(1)}%</strong>
                  </p>
                )}
              </div>
            </div>
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="text-sm text-muted-foreground space-y-1">
                    <p>
                      <strong>Solicitado:</strong> {formatDate(repasse.criado_em)}
                    </p>
                    {repasse.data_repasso && (
                      <p>
                        <strong>Pago:</strong> {formatDate(repasse.data_repasso)}
                      </p>
                    )}
                    {repasse.metodo && (
                      <p>
                        <strong>Método:</strong> {repasse.metodo}
                      </p>
                    )}
                    {repasse.observacao && (
                      <p>
                        <strong>Obs:</strong> {repasse.observacao}
                      </p>
                    )}
                  </div>
                </div>
...
export default SolicitacaoRepasse;
